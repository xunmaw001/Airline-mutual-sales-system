	...
	<div class="detail-preview" :style='{"margin":"0px auto","alignItems":"flex-start","flexWrap":"wrap","background":"none","display":"flex","width":"100%","position":"relative","justifyContent":"space-between"}'>
		<div class="attr" :style='{"padding":"0","overflow":"hidden","borderRadius":"4px","flexWrap":"wrap","background":"none","display":"flex","width":"100%","position":"relative","justifyContent":"space-between"}'>
			...
			<div class="info" :style='{"width":"86%","padding":"10px 20px","margin":"20px 7% 20px","background":"url(http://codegen.caihongy.cn/20230127/03ad723288d44044a8b922de0a51127f.png) no-repeat left top,url(http://codegen.caihongy.cn/20230127/e1b074885e304a40bcfa7de91c22b700.png) no-repeat center bottom,#e0edf2"}'>
				...
				<div class="item" :style='{"border":"0 dashed #abd0e3","padding":"10px","margin":"0 auto 10px","background":"none","borderWidth":"0 0 1px","display":"flex","width":"90%","justifyContent":"spaceBetween"}'>
					<div class="lable" :style='{"padding":"0 10px 0 0","color":"#2886bc","textAlign":"right","width":"auto","fontSize":"16px","lineHeight":"40px","minWidth":"130px","height":"40px"}'>商品简介</div>
					<div  :style='{"width":"100%","padding":"8px 10px 0","fontSize":"14px","lineHeight":"24px","color":"#333","height":"24px"}'>{{detail.shangpinjianjie}}</div>
				</div>
				<div class="item" :style='{"border":"0 dashed #abd0e3","padding":"10px","margin":"0 auto 10px","background":"none","borderWidth":"0 0 1px","display":"flex","width":"90%","justifyContent":"spaceBetween"}'>
					<div class="lable" :style='{"padding":"0 10px 0 0","color":"#2886bc","textAlign":"right","width":"auto","fontSize":"16px","lineHeight":"40px","minWidth":"130px","height":"40px"}'>商品描述</div>
					<div  :style='{"width":"100%","padding":"8px 10px 0","fontSize":"14px","lineHeight":"24px","color":"#333","height":"24px"}'>{{detail.shangpinmiaoshu}}</div>
				</div>
				<div class="item" :style='{"border":"0 dashed #abd0e3","padding":"10px","margin":"0 auto 10px","background":"none","borderWidth":"0 0 1px","display":"flex","width":"90%","justifyContent":"spaceBetween"}'>
				...
				<div class="btn" :style='{"padding":"10px 7%","flexWrap":"wrap","display":"flex"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#333","borderRadius":"4px","background":"#b9f4e3","width":"auto","lineHeight":"36px","fontSize":"14px","height":"36px"}' v-if="isAuth('shangpinhushou','审核')" @click="onSH" type="warning">审核</el-button>
				</div>
			</div>
  ...
  export default {
    ...
    methods: {
      ...
      storeup(type) {
        ...
        if (type == -1 && this.isStoreup) {
          this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: 1, refid: this.detail.id, tablename: 'shangpinhushou', userid: localStorage.getItem('userid')}}).then(res => {
      ...
      getDiscussList(page) {
        this.$http.get('discussshangpinhushou/list', {params: {page, limit: this.pageSize, refid: this.detail.id}}).then(res => {
          if (res.data.code == 0) {
            ...
            this.$http.get('orders/list', {params: {page: 1, limit: 1, status: '已完成', goodid: this.detail.id, userid: localStorage.getItem('userid')}}).then(res => {
                if (res.data.code == 0 && res.data.data.list.length == 0) {
                  ...
                  return false
                    ...
                    this.$http.post('discussshangpinhushou/add', this.form).then(res => {
                      if (res.data.code == 0) {
                        this.form.content = '';
                        this.getDiscussList(1);
                        this.$message({
      ...
      getCartList() {
        this.$http.get('cart/list', {params: {userid: localStorage.getItem('userid'), tablename: 'shangpinhushou', goodid: this.detail.id}}).then(res => {
          if (res.data.code == 0) {
